...
uninstall() {
 if [ ! -f "$WECHAT_EXECUTABLE_ORIGINAL_PATH" ]; then
   echo "[${X1A0HE_WECHAT_PLUGIN_INSTALLER}] 未检测到已安装的插件"
   exit 0
 fi
 
 echo "[${X1A0HE_WECHAT_PLUGIN_INSTALLER}] 正在卸载插件..."
 ...
 # 删除插件文件
 rm -f "$WECHAT_APP_PATH/X1a0HeWeChatPlugin.dylib"
 
 # 重新签名恢复后的可执行文件
 sudo /usr/bin/codesign -f -s - --all-architectures "$WECHAT_EXECUTABLE_PATH"
 
 echo "[${X1A0HE_WECHAT_PLUGIN_INSTALLER}] 插件已成功卸载"
 exit 0
}
